...
- **`.claude/skills/biblethought-maintenance/scripts/generate_index.py`**: walks the entire repo and regenerates both `index.md` and `_data/nav.json`. Run it manually with `python3 .claude/skills/biblethought-maintenance/scripts/generate_index.py` if needed.
...
| File | Purpose |
|------|---------|
| `.claude/skills/biblethought-maintenance/scripts/generate_index.py` | Regenerates `index.md` + `nav.json`; runs automatically on every commit |
